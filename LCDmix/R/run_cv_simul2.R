...
run_cv_simul2 <- function(
  ...
  for (s in seq_len(S)) {
    ...
    if (grepl("\\.rds$", f, ignore.case = TRUE)) {
      obj <- readRDS(f); if (is.list(obj)) utils::list2env(obj, env) else stop("RDS must be a list with Y_bin, X, bin_mass.")
    } else {
      load(f, envir = env)
    }
    ...
    save(env$index_matrix, file = file.path(sim_dir, "index_matrix.Rdata"))
    sims[[s]] <- env
  }
