...
#' corresponding rows, calls \code{cv_summary()} to aggregate the CV results for
...
#'   columns required by your \code{cv_summary()} are passed along after removing
#'   \code{sim_idx}.
...
#' # logs <- run_cv_simul(idx_mat, sim_dir="sim_data", K=2, save_dir="cv_results")
#'
...
summarize_simul_cv <- function(simul_idx_mat, save_dir) {
  ...
  for (sim in sim_idxs) {
    ...
    out     <- cv_summary(idx_mat, save_dir, simul = TRUE, sim_idx = sim)
